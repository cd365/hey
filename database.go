...
type QueryJoin interface {
	...
	SelectGroupsColumns(columns ...[]string) QueryColumns

	// SelectTableColumnAlias Batch set multiple columns of the specified table and set aliases for all columns.
	SelectTableColumnAlias(table TableCmder, columnAndColumnAlias ...string) QueryColumns
}
...
func (s *queryJoin) TableSelect(table TableCmder, columns ...string) []string {
	if table != nil && len(columns) > 0 {
		if aliasName := table.GetAlias(); aliasName != EmptyString {
			return s.way.T().SetAlias(aliasName).ColumnAll(columns...)
		}
	}
	return nil
}

func (s *queryJoin) SelectGroupsColumns(columns ...[]string) QueryColumns {
	groups := make([]string, 0, 1<<5)
	for _, values := range columns {
		groups = append(groups, values...)
	}
	return s.queryColumns.AddAll(groups...)
}

func (s *queryJoin) SelectTableColumnAlias(table TableCmder, columnAndColumnAlias ...string) QueryColumns {
	length := len(columnAndColumnAlias)
	if table == nil || length == 0 || length&1 != 0 {
		return s.queryColumns
	}
	if aliasName := table.GetAlias(); aliasName != EmptyString {
		t := s.way.T().SetAlias(aliasName)
		for i := 0; i < length; i += 2 {
			if columnAndColumnAlias[i] != EmptyString && columnAndColumnAlias[i+1] != EmptyString {
				s.queryColumns.Add(t.Column(columnAndColumnAlias[i], columnAndColumnAlias[i+1]))
			}
		}
	}
	return s.queryColumns
