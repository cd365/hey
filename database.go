...
type Result struct {
	TimeStart int64         // the time to start executing SQL
	TimeEnd   int64         // the time at which the execution of SQL ends
	Prepare   string        // sql prepare
	Args      []interface{} // sql args
	Error     error         // error messages generated by executing SQL
	TxId      string        // database transaction unique id
}
...
func (s *Way) QueryContext(ctx context.Context, handlingRows func(rows *sql.Rows) (err error), prepare string, args ...interface{}) (err error) {
	...
	if err != nil {
		if result != nil {
			result.TimeEnd = time.Now().UnixNano()
		}
		return
	}
	defer stmt.Close()
