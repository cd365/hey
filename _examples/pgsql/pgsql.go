...
func MyMulti() {
	...
	exists := false
	m.AddQueryExists(m.V().Table(department).ToExists(), &exists)
	...
	queryRow := m.V().Table(department).Select(department.Id, department.Name).Limit(1).ToSelect()
	...
	// With window alias
	{
		...
		query := way.Table(employee).Alias(a.Table()).LeftJoin(func(join hey.SQLJoin) (hey.SQLAlias, hey.SQLJoinOn) {
			...
			join.Select(
				...
				way.WindowFunc("min_salary").Min(ac(employee.DepartmentId)).Over(wa),
			)
			join.Select(
				bc(department.Name, "department_name"),
			)
		...
		query.Window(wa, func(o hey.SQLWindowFuncOver) {
			o.Partition(ac(employee.DepartmentId))
			o.Desc(ac(employee.Id))
		})
		query.WhereFunc(func(f hey.Filter) {
